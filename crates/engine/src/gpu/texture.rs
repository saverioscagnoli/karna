...
impl Texture {
        ...
        L: AsRef<str>,
    {
        ...
        let raw = unsafe {
            ...
            let Some(tex) = NonNull::new(SDL_CreateGPUTexture(device.raw(), &info)) else {
                panic!("Failed to create texture '{}': {}", label, sdl_last_error());
            };

            if let Ok(c) = CString::new(label) {
                SDL_SetGPUTextureName(device.raw(), tex.as_ptr(), c.as_ptr());
            }
    ...
    pub fn white(device: Device) -> Self {
        let texture = Self::new(device.clone(), "white", TextureDesc::rgba8(1, 1));
...
impl Drop for Texture {
    fn drop(&mut self) {
        unsafe { SDL_ReleaseGPUTexture(self.device.raw(), self.raw.as_ptr()) }
    }
}
...
impl Sampler {
    pub fn new(device: Device, desc: SamplerDesc) -> Self {
        let raw = unsafe {
            ...
            let Some(sampler) = NonNull::new(SDL_CreateGPUSampler(device.raw(), &info)) else {
                panic!("Failed to create sampler: {}", sdl_last_error());
            };
...
impl Drop for Sampler {
    fn drop(&mut self) {
        unsafe { SDL_ReleaseGPUSampler(self.device.raw(), self.raw.as_ptr()) }
    }
}
...
fn sdl_err() -> BufferError {
    BufferError::SDL(format!("{}", sdl_last_error()))
}
