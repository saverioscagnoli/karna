...
impl<V> Batcher<V> {
    pub fn new(pipeline: wgpu::RenderPipeline) -> Self {
        let vertex_buffer = GpuBufferBuilder::new()
            ...
            .capacity(consts::IMMEDIATE_VERTICES_STEP)
            .vertex()
            .copy_dst()
            .build();
        ...
        let index_buffer = GpuBufferBuilder::new()
            .label("Immediate Vertex Buffer")
            .capacity(consts::IMMEDIATE_INDICES_STEP)
            .index()
            .copy_dst()
            .build();

        Self {
            vertices: Vec::with_capacity(consts::IMMEDIATE_VERTICES_STEP),
            indices: Vec::with_capacity(consts::IMMEDIATE_INDICES_STEP),
            vertex_buffer,
            index_buffer,
            pipeline,
        }
