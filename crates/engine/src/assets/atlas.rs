...
use crate::gpu::texture::TextureDesc;
...
impl Page {
    fn new(extent: u32, kind: PageKind, filter: Filter) -> Self {
        Self {
            ...
            packer: PagePacker::new(extent, config().atlas_padding),
            kind,
            filter,
            texture: None,
            uploaded: 0,
...
impl TextureAtlas {
    ...
    fn padding() -> u32 {
        config().atlas_padding
    }

    fn page_size() -> u32 {
        config().atlas_page_size
    }
    ...
    pub fn upload_dirty(&mut self, gpu: &Rc<Gpu>) {
        let dirty = self
            .pages
            .iter()
            .enumerate()
        ...
        if let Err(err) = gpu.upload_textures(&uploads) {
