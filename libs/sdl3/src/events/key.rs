...
macro_rules! keys {
    ($($name:ident => $sc:ident),* $(,)?) => {
        ...
        impl Key {
            ...
            pub fn from_scancode(raw: SDL_Scancode) -> Option<Self> {
                match raw {
                    $(::sdl3_sys::$sc => Some(Self::$name),)*
                    _ => None,
                }
            }

            /// The raw SDL scancode behind this key.
            pub fn to_scancode(self) -> SDL_Scancode {
                self as u32 as SDL_Scancode
            }
