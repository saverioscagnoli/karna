...
impl Default for FpsCounter {
    fn default() -> Self {
        ...
        Self {
            strategy: config.time.fps_calc_strategy,
            sample_size: config.time.fps_sample_size,
            samples: VecDeque::with_capacity(config.time.fps_sample_size),
            sum: Duration::ZERO,
            ema: None,
        }
    }
...
impl FpsCounter {
    pub fn push(&mut self, delta: Duration) {
        match self.strategy {
            ...
            FpsCalcStrategy::Smoothed => {
                let config = config();
                let dt = delta.as_secs_f32();
                let alpha = 1.0 - (-dt / config.time.fps_smoothing_tau).exp();
    ...
    pub fn avg_frame_time(&self) -> Option<Duration> {
        match self.strategy {
            ...
            FpsCalcStrategy::Smoothed { .. } => self.ema.map(Duration::from_secs_f32),
        }
    }
    ...
    pub fn set_strategy(&mut self, strategy: FpsCalcStrategy) {
...
impl FramePacer {
    pub fn new(mode: PaceMode) -> Self {
        let conf = config();
        let rate = Duration::from_secs_f32(1.0 / conf.time.target_fps as f32);
