...
impl<T> GpuBuffer<T> {
    pub fn new<L>(gpu: *mut SDL_GPUDevice, label: L, capacity: usize, usage: BufferUsage) -> Self
    where
        L: AsRef<str>,
    {
        const {
        ...
        let raw = unsafe {
            ...
            let buf =
                NonNull::new(SDL_CreateGPUBuffer(gpu, &info)).expect("Failed to create buffer");

            if let Ok(c) = std::ffi::CString::new(label) {
                SDL_SetGPUBufferName(gpu, buf.as_ptr(), c.as_ptr());
            }
    ...
    pub fn reserve(&mut self, capacity: usize) {
        ...
        *self = Self::new(gpu, label, grown.next_power_of_two(), usage);
    }
}
...
impl<T> Drop for GpuBuffer<T> {
    fn drop(&mut self) {
        unsafe {
            SDL_ReleaseGPUBuffer(self.gpu, self.raw.as_ptr());
        }
    }
}
