...
pub struct FramePacket {
    pub world: Vec<DrawCommand>,
    pub ui: Vec<DrawCommand>,
    pub debug: Vec<DrawCommand>,
}
...
impl Renderer {
    ...
    fn draw_layer(&mut self, commands: Vec<DrawCommand>) {
        for command in commands {
            match command {
                DrawCommand::ImmediateRect { x, y, w, h } => {}
                _ => {}
            }
        }
    }

    pub fn present(&mut self, gpu: &GpuState, surface: &gpu::WindowSurface, packet: FramePacket) {}
}
